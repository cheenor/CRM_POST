      program read_tape 
      PARAMETER (MY=202,MZ=52,mym=my-1, im=202,km=52,nt=2880*12)
...
C     *,FORM='UNFORMATTED')
     ...
     *'micro_'//trim(casenm)//'.txt')
      open(74,file=trim(path)//
     *'surface_'//trim(casenm)//'.txt')

      NVLT=1
      DO 8888 NIOSTR=1,35016
c     DO 8888 NIOSTR=1,2592
 ...
 28   FORMAT('WUTAPE_',A17)
      write(WU2_IFNAME,29)IFNAME
 29   FORMAT('RADTAPE_',A17)
